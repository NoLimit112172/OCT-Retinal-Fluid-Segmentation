...
from model_nnunet_sda import nnUNet
...
# 实验 5：nnU-Net + SDA，单独目录，避免覆盖其他实验
CHECKPOINT_DIR = 'checkpoints_nnunet_sda'
...
def calculate_val_dice(pred_logits, target, num_classes):
    """
    与前面 base / ECA / CBAM 训练脚本保持一致，用于保存 best_model。
...
def train():
    print(f'🔥 [nnU-Net + SDA] 准备训练... 设备: {DEVICE}')
