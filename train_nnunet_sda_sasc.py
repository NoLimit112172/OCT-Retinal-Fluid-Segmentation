...
from model_nnunet_sda_sasc import nnUNet
...
CHECKPOINT_DIR = 'checkpoints_nnunet_sda_sasc'
...
def calculate_val_dice(pred_logits, target, num_classes):
    """
    用于训练期间保存 best_model。
    最终论文表格建议用 evaluate_comparison_corrected.py 统一重新评估。
    """
    dice_scores = []
...
def train():
    print(f'🔥 [nnU-Net + SDA/SASC v2] 准备训练... 设备: {DEVICE}')
    ...
    # 每轮平滑下降，避免 ReduceLROnPlateau 一直不触发导致日志里 LR 不变。
    ...
    if RESUME and os.path.exists(LAST_CKPT_PATH):
        ...
        print('🆕 从头开始训练新的 nnU-Net + SDA/SASC  实验。')
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            ...
            if avg_val_dice > best_dice:
                ...
                print(f'🏆 新纪录！SDA/SASC v2 最佳模型已保存: {BEST_MODEL_PATH} (Dice: {best_dice:.4f})')
