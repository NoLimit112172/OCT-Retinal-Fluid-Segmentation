...
# ================= 所有模型配置 =================
# 包含：
# 1. 五个基线模型：SegNet / U-Net / Attention U-Net / DeepLabV3+ / nnU-Net Base
# 2. nnU-Net 系列消融模型：ECA / CBAM / ECA-CBAM / SDA / SASC / SDA-SASC
MODELS_TO_EVALUATE = [
    # ---------- baseline models ----------
    {
        "name": "SegNet",
...
def evaluate_model(conf: Dict[str, Any], dataloader: DataLoader) -> Optional[Dict[str, float]]:
    ...
    row["Mean_3Fluid_Dice"] = float(np.mean([
        row["IRF_Dice"],
        row["SRF_Dice"],
        row["PED_Dice"],
    ]))

    row["Mean_3Fluid_IoU"] = float(np.mean([
        row["IRF_IoU"],
        row["SRF_IoU"],
        row["PED_IoU"],
    ]))
    ...
    print(
        ...
        f"三类Dice均值: {row['Mean_3Fluid_Dice']:.4f} | "
        f"三类IoU均值: {row['Mean_3Fluid_IoU']:.4f}"
    )
...
def reorder_columns(df: pd.DataFrame) -> pd.DataFrame:
    preferred_cols = [
        "Model", "Group",
        "Dice", "IoU",
        "Mean_3Fluid_Dice", "Mean_3Fluid_IoU",
        "Precision", "Recall",
...
def plot_mean_dice_iou(df: pd.DataFrame, output_name: str):
    ...
    fig, ax = plt.subplots(figsize=(12, max(6.0, len(models) * 0.6)))
...
def plot_per_fluid(df: pd.DataFrame, metric: str, output_name: str):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(plot_df["Model"], rotation=28, ha="right")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel(metric)
    ax.set_title(f"所有模型 IRF / SRF / PED {metric} 对比")
...
def plot_grouped_overview(df: pd.DataFrame, output_name: str):
    """
    按 Baseline / Ablation 分组展示 Dice。
    """
    ...
    plot_df["Group_Order"] = plot_df["Group"].map({
        "Baseline": 0,
        "Baseline / Ablation Base": 1,
        "Ablation": 2,
    }).fillna(9)

    plot_df = plot_df.sort_values(["Group_Order", "Dice"], ascending=[True, False])

    fig, ax = plt.subplots(figsize=(13, 6.2))
    x = np.arange(len(plot_df))

    bars = ax.bar(x, plot_df["Dice"])

    ax.set_xticks(x)
    ax.set_xticklabels(plot_df["Model"], rotation=28, ha="right")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Dice")
    ax.set_title("所有模型 Dice 总览")
    ax.grid(axis="y", linestyle="--", alpha=0.35)
...
def main():
    ...
    out_csv = "all_models_results_oldstyle.csv"
    df.to_csv(out_csv, index=False, encoding="utf-8-sig")
    print(f"💾 表格已保存: {out_csv}")

    # 兼容 GUI：如需让 GUI 直接读取所有模型结果，可取消下一行注释
    # df.to_csv("model_comparison_results.csv", index=False, encoding="utf-8-sig")

    plot_mean_dice_iou(df, "all_models_mean_dice_iou.png")
    plot_per_fluid(df, "Dice", "all_models_per_fluid_dice.png")
    plot_per_fluid(df, "IoU", "all_models_per_fluid_iou.png")
    plot_grouped_overview(df, "all_models_dice_overview.png")
