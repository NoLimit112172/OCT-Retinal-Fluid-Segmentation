...
from model_nnunet_sasc import nnUNet
...
# 最终仍然使用 32 通道基准，不是 16。
BASE_CHANNELS = 32

# 实验：nnU-Net + SASC
CHECKPOINT_DIR = 'checkpoints_nnunet_sasc'
...
def set_seed(seed=2024):
    ...
    # 为了速度和稳定性，这里不强行设置 deterministic=True
...
class OCTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 与你当前训练/评估体系保持一致：超出 0~3 的标签映射为背景
...
def calculate_val_dice(pred_logits, target, num_classes):
    """
    用于训练过程中选择 best_model。
    这里保持你之前训练脚本的 OldStyle 风格：空类别记为 1.0，
    这样和你前面几个模型的 best_model 选择方式一致。
    最终论文结果仍然建议用 evaluate_comparison_oldstyle_no_aspp.py 统一评估。
    """
    dice_scores = []
...
def train():
    set_seed(SEED)
    print(f'🔥 [nnU-Net + SASC] 准备训练... 设备: {DEVICE}')
    ...
    # 为了和你前面 base / ECA / CBAM / SDA 实验尽量公平，
    # 这里默认沿用 ReduceLROnPlateau，而不是强行换成 Cosine。
    ...
    if RESUME and os.path.exists(LAST_CKPT_PATH):
        ...
        print('🆕 从头开始训练新的 nnU-Net + SASC 实验。')
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            ...
            if avg_val_dice > best_dice:
                ...
                print(f'🏆 新纪录！SASC 最佳模型已保存: {BEST_MODEL_PATH} (Dice: {best_dice:.4f})')
