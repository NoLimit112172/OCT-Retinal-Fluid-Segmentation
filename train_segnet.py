...
# 与 U-Net / Attention U-Net / DeepLabV3+ 保持一致：默认从头训练
RESUME = False
CHECKPOINT_DIR = 'checkpoints_dicece_segnet'
...
def set_seed(seed=42):
    ...
    # 与你现有 U-Net / Attention U-Net 训练代码保持一致
...
class OCTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 与 U-Net / Attention U-Net 保持一致：非法标签映射为背景
...
def calculate_dice(pred, target, num_classes):
    """
    与 U-Net / Attention U-Net 训练脚本一致：
    - 不计算背景；
    - 空类别预测正确时 Dice 记为 1.0；
    - 用于训练阶段保存 best_model。
    """
    dice_scores = []
...
def train():
    set_seed(SEED)
    print(f"🔥 [SegNet] Dice + CE 训练开始... 设备: {DEVICE}")
    ...
    model = SegNet(n_channels=1, n_classes=NUM_CLASSES).to(DEVICE)
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            ...
            pbar = tqdm(
                train_loader,
                desc=f"Epoch {epoch + 1}/{EPOCHS} [LR={current_lr:.1e}]"
            )
            ...
            print(
                f"✅ [SegNet] Epoch {epoch + 1} | Total Loss: {avg_train_loss:.4f} "
            ...
            if avg_val_dice > best_dice:
                ...
                print(f"🏆 [SegNet] 新纪录！最佳模型已保存 (Dice: {best_dice:.4f})")
