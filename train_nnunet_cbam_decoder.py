...
from model_nnunet_cbam_decoder import nnUNet
...
# 实验 4：nnU-Net + CBAM-Decoder，单独目录，避免覆盖其他实验
CHECKPOINT_DIR = 'checkpoints_nnunet_cbam_decoder'
...
class OCTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 防止 mask 里出现超出类别范围的像素值导致 CrossEntropyLoss 报错
...
def calculate_val_dice(pred_logits, target, num_classes):
    """
    与前面 base / ECA 训练脚本保持一致，用于保存 best_model。
    最终论文表格建议使用 evaluate_comparison_corrected.py 统一评估。
    """
    dice_scores = []
...
def train():
    print(f'🔥 [nnU-Net + CBAM-Decoder] 准备训练... 设备: {DEVICE}')
    ...
    model = nnUNet(n_channels=1, n_classes=NUM_CLASSES).to(DEVICE)
    ...
    if RESUME and os.path.exists(LAST_CKPT_PATH):
        ...
        print('🆕 从头开始训练新的 nnU-Net + CBAM-Decoder 实验。')
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            ...
            if avg_val_dice > best_dice:
                ...
                print(f'🏆 新纪录！最佳模型已保存 (Dice: {best_dice:.4f})')
